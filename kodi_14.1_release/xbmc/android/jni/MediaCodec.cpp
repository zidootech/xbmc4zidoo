...
void CJNIMediaCodec::configure(const CJNIMediaFormat &format, CJNISurfaceView &surfaceView, const CJNIMediaCrypto &crypto, int flags)
{
	call_method<void>(m_object, "configure", "(Landroid/media/MediaFormat;Landroid/view/Surface;Landroid/media/MediaCrypto;I)V", format.get_raw(), surfaceView.getSurface().get_raw(), crypto.get_raw(), flags);
}
