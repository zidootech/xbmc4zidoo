    ...
    case AV_CODEC_ID_HEVC:
      {
        ...
        uint8_t numOfArrays;
        m_extrasize = 0;
        static uint8_t  hevccfgprefix[3] = {0, 0, 1};

        if (in_extrasize < 4 || (in_extradata[0] == 0 && in_extradata[1] == 0 && in_extradata[2] == 1)
          || (in_extradata[0] == 0 && in_extradata[1] == 0 && in_extradata[2] == 0 && in_extradata[3] == 1)) {
          CLog::Log(LOGINFO, "CBitstreamConverter::Open don't neet to convert");
          m_convert_bytestream = false;
        } else {
          m_offset += 21; //skip 21 bytes
          /* 1 byte configurationVersion */
          /* 1 byte , 2 bit profile_space, 1 bit tier_flag, 5 bit profile_idc */
          /* 4 byte profile_compatibility_flags */
          /* 6 byte constraint_indicator_flagss */
          /* 1 byte level_idc */
          /* 2 byte min_spatial_segmentation_idc */
          ...
          data = in_extradata[m_offset++];

          lengthSizeMinusOne = data & 0x3;
          hevc_nal_unit_size = lengthSizeMinusOne + 1;
          numOfArrays = in_extradata[m_offset++];

          m_extradata = (uint8_t*)av_malloc(in_extrasize);
          for (int i = 0; i < numOfArrays && m_offset < in_extrasize; i++)
          {
            uint16_t numNalus = 0;
            /* 1 byte, 1 bit array_completeness, 1 bit reserved, 6 bit NAL_unit_type */
            m_offset++;
            /* 2 byte nalus value*/
            numNalus = in_extradata[m_offset++] << 8;
            numNalus |= in_extradata[m_offset++];
            for (int j=0; j< numNalus && m_offset < in_extrasize; j++) {
              uint16_t ps_length = 0;
              ps_length = in_extradata[m_offset++] << 8;
              ps_length |= in_extradata[m_offset++];
              uint32_t read_size = ps_length <= in_extrasize - m_offset ? ps_length : in_extrasize - m_offset;

              memcpy(m_extradata + m_extrasize, hevccfgprefix, sizeof(hevccfgprefix));
              m_extrasize += sizeof(hevccfgprefix);

              memcpy(m_extradata + m_extrasize, in_extradata + m_offset, read_size);
              m_offset += read_size;
              m_extrasize += read_size;
            }
          }
          m_convert_bytestream = true;
        }
...
bool CBitstreamConverter::Convert(uint8_t *pData, int iSize)
{
  ...
  if (pData)
  {
    if (m_codec == AV_CODEC_ID_H264)
    {
      ...
      if (m_convert_bytestream && iSize) {
        if (hevc_nal_unit_size > 4) {
          m_convertBuffer = NULL;
          return false;
        }

        pNewData = (uint8_t *)av_malloc(iSize);

        while(m_readoffset < iSize) {
          if (iSize - m_readoffset < hevc_nal_unit_size) {
            CLog::Log(LOGERROR, "CBitstreamConverter::Convert: check nal byte count.");
            bsuccess = false;
            break;
          }
          //Read nal size
          for (int i = 0; i < hevc_nal_unit_size; i++)
          {
            nal_size <<= 8;
            nal_size |= pData[m_readoffset++];
          }

          memcpy(pNewData + payloadLen, hevccfgprefix, sizeof(hevccfgprefix));
          payloadLen += sizeof(hevccfgprefix);
          memcpy(pNewData + payloadLen, pData + m_readoffset, nal_size);
          m_readoffset += nal_size;
          payloadLen += nal_size;
        }

        if(bsuccess) {
          m_convertBuffer = pNewData;
          m_convertSize = payloadLen;
        } else {
          m_convertBuffer = NULL;
          m_convertSize = 0;
          av_free(pNewData);
