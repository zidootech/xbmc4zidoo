...
bool CDVDVideoCodecAndroidMediaCodec::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options)
{
  ...
  switch(m_hints.codec)
  {
    ...
    case AV_CODEC_ID_RV40:
      m_mime = "video/realvideo";
      m_formatname = "amc-realvideo";
      break;
...
int CDVDVideoCodecAndroidMediaCodec::Decode(uint8_t *pData, int iSize, double dts, double pts)
{
  ...
  if (m_hints.ptsinvalid)
    ...
    int ret;
    if(m_getPictureCount == 0)
    {
        // Get the first picture fast
        ret = GetOutputPicture(5000);
    }
    else
    {
        ret = GetOutputPicture(50000);
        // the decoder don't needs more data
        if(m_pushPictureCount - m_getPictureCount > 20)
        {
            rtn &= ~VC_BUFFER;
        }
    }
...
bool CDVDVideoCodecAndroidMediaCodec::ConfigureMediaCodec(void)
{
  ...
  // push blank buffers on stop
  mediaformat.setInteger(CJNIMediaFormat::KEY_PUSH_BLANK_BUFFERS_ON_STOP, 1);
  ...
  {
#ifdef RENDER_WITH_SURFACEVIEW
	CLog::Log(LOGERROR,"render with surfaceview");
    m_codec->configure(mediaformat, *m_surfaceView, crypto, flags);
#else
	CLog::Log(LOGERROR,"render with surfacetexture");
...
int CDVDVideoCodecAndroidMediaCodec::GetOutputPicture(int64_t timeout)
{
  int rtn = 0;
  ...
  if (index >= 0)
  {
    ...
    if (!m_render_sw)
    {
      // Add protect for decode fail
      if ((int)m_inflight.size() > index && index >= 0)
      {
