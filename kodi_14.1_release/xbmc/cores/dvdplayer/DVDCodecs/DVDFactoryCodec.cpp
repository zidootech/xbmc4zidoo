...
#if defined(TARGET_ANDROID)
    if (!hint.software && CSettings::Get().GetBool("videoplayer.usemediacodec") && hint.width>640 && hint.height>=480)
    {
...
#if defined(HAS_LIBSTAGEFRIGHT)
    if (!hint.software && CSettings::Get().GetBool("videoplayer.usestagefright") && hint.width>640 && hint.height>=480)
    {
      switch(hint.codec)
      {
        case CODEC_ID_H264:
