...
// MSTAP PATCH BEGIN
      ...
      if (!m_pAudioCodec->NeedPassthrough()) {
      	   len = m_pAudioCodec->Decode(m_decode.data, m_decode.size);
      } else {
          len = m_pAudioCodec->Decode(m_decode.data, m_decode.size, dts, m_decode.duration);
      }
// MSTAR PATCH END
...
// MSTAR PATCH BEGIN
      if (m_pAudioCodec->NeedPassthrough()) {
          int remain = m_decode.size - len;
          if (remain > 0)
              m_decode.duration = (m_decode.duration * remain)/m_decode.size;
          else
          	   m_decode.duration = 0;
      }
// MSTAR PATCH END
    ...
    if (pMsg->IsType(CDVDMsg::DEMUXER_PACKET))
    {
      ...
      //CLog::Log(LOGDEBUG, "CDVDPlayerAudio - CDVDMsg::DEMUXER_PACKET,size:%d, data:0x%x %x %x %x, pts:%f, dts:%f, duration:%f",
      	         //m_decode.size, (m_decode.data)[0], (m_decode.data)[1], (m_decode.data)[2], (m_decode.data)[3], m_decode.pts, m_decode.dts, m_decode.duration);
    }
