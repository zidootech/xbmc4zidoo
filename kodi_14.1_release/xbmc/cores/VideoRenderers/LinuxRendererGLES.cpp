...
void CLinuxRendererGLES::RenderUpdate(bool clear, DWORD flags, DWORD alpha)
{
  ...
  if ((m_renderMethod & RENDER_BYPASS) || (m_renderMethod & RENDER_MEDIACODEC))
#else
...
#ifdef RENDER_WITH_SURFACEVIEW
#ifdef DEBUG_VERBOSE
        static unsigned int time = XbmcThreads::SystemClockMillis();
        CLog::Log(LOGDEBUG,"[ReleaseBuffer][index:%d][mediacodec_index:%d][count:%d][interval:%u]", idx, buf.mediacodec->GetIndex(), m_releaseBufferCount, XbmcThreads::SystemClockMillis() - time);
        time = XbmcThreads::SystemClockMillis();
#endif
        buf.mediacodec->ReleaseOutputBuffer(TRUE);
        buf.mediacodec->Validate(FALSE);
#else
